...
void ReactionsSet::addReaction(Eigen::MatrixXi reaction, double k,
  ...
  descriptions.push_back(reaction_id);  // add reaction's description. e.g.: in
                                        // a mRNA it would be the codon.
}
...
/**
 * @brief Given the species (reactants), calculate An for all n reactions. This
 * method is an optimized version of the cannonical one: we only look into the
 * valid reactions (e.g.: where k != 0 or k*population !=0).
 *
 * @param species The Matrix with the reactants
 * @param as_vector Eigen vector to store the An value (for 1st order reactions:
 * k*population, for 0-order reactions: k), where k = reaction's propensity.
 * @param reaction_number_vector Eigen vector to contain the index of the
 * reaction in the reactionsSet object.
 */
void ReactionsSet::getAlphas(const Eigen::MatrixXi& species,
                             std::vector<double>& as,
                             std::vector<int>& reaction_number) {
  as.clear();
  reaction_number.clear();
  int i = 0;
  for (std::tuple<double, std::vector<std::tuple<int, int>>> k_and_index :
       k_pop_index) {
    ...
    if (std::get<1>(k_and_index).empty()) {
      ...
      for (std::tuple<int, int> element : std::get<1>(k_and_index))
        specie_population *=
            species(std::get<0>(element), std::get<1>(element));
      // store the reaction's propensity and the locations of the reactants in
      // the species matrix.
...
Eigen::MatrixXi ReactionsSet::getReaction(int index) {
  return reactions_vector.at(static_cast<std::size_t>(index));
}
